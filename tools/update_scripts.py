    ...
    for pattern in patterns:
        ...
        if match:
            ...
            if (len(manufacturer) >= 2 and 
                len(model_clean) >= 2 and  # Reduziert auf 2 für abgekürzte Namen
                not any(word in text.lower() for word in ['script', 'view', 'example', 'alternative']) and
                not model_clean.lower() in ['xxx', 'tbd', 'etc', 'und']):  # Keine Platzhalter
